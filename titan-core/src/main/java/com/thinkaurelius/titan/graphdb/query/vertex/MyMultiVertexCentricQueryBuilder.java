 ...
 * {@link #execute(com.thinkaurelius.titan.graphdb.internal.RelationCategory, MyBasicVertexCentricQueryBuilder.ResultConstructor)}.
 * </p>
 * All other methods just prepare or transform that result set to fit the particular method semantics.
 *
 * @author Matthias Broecheler (me@matthiasb.com)
 */
public class MyMultiVertexCentricQueryBuilder extends MyBasicVertexCentricQueryBuilder<MyMultiVertexCentricQueryBuilder> implements TitanMultiVertexQuery<MyMultiVertexCentricQueryBuilder> {
    ...
    public MyMultiVertexCentricQueryBuilder(final StandardTitanTx tx) {
        super(tx);
        vertices = Sets.newHashSet();
    }

    @Override
    protected MyMultiVertexCentricQueryBuilder getThis() {
        return this;
    }
    ...
    protected<Q> Map<TitanVertex,Q> execute(RelationCategory returnType, ResultConstructor<Q> resultConstructor) {
        ...
        MyBaseVertexCentricQuery bq = super.constructQuery(returnType);
        ...
        if (!bq.isEmpty()) {
            for (BackendQueryHolder<EdgeSliceQuery> sq : bq.getQueries()) {
