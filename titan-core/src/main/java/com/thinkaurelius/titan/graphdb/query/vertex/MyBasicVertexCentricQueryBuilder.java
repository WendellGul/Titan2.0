 ...
 */
public abstract class MyBasicVertexCentricQueryBuilder<Q extends BaseVertexQuery<Q>> extends BaseVertexCentricQueryBuilder<Q> {
    ...
    /**
     ...
     This is achieved by using the {@link MySimpleVertexQueryProcessor} for execution.
     */
    ...
    public MyBasicVertexCentricQueryBuilder(final StandardTitanTx tx) {
        super(tx);
    ...
    protected interface ResultConstructor<Q> {
        ...
        Q getResult(InternalVertex v, MyBaseVertexCentricQuery bq);
    ...
    protected class RelationConstructor implements ResultConstructor<Iterable<? extends TitanRelation>> {
        ...
        @Override
        public Iterable<? extends TitanRelation> getResult(InternalVertex v, MyBaseVertexCentricQuery bq) {
    ...
    protected class VertexConstructor implements ResultConstructor<Iterable<TitanVertex>> {
        ...
        @Override
        public Iterable<TitanVertex> getResult(InternalVertex v, MyBaseVertexCentricQuery bq) {
    ...
    protected class VertexIdConstructor implements ResultConstructor<VertexList> {
        ...
        @Override
        public VertexList getResult(InternalVertex v, MyBaseVertexCentricQuery bq) {
    ...
    protected boolean useSimpleQueryProcessor(MyBaseVertexCentricQuery query, InternalVertex... vertices) {
    ...
    protected Iterable<TitanRelation> executeRelations(InternalVertex vertex, MyBaseVertexCentricQuery baseQuery) {
    ...
    private Iterable<TitanRelation> executeIndividualRelations(InternalVertex vertex, MyBaseVertexCentricQuery baseQuery) {
        MyVertexCentricQuery query = constructQuery(vertex, baseQuery);
        if (useSimpleQueryProcessor(query,vertex))
            return new MySimpleVertexQueryProcessor(query,tx).relations();
        else
            return new QueryProcessor<MyVertexCentricQuery, TitanRelation, EdgeSliceQuery>(query, tx.edgeProcessor_);
    }

    public Iterable<TitanVertex> executeVertices(InternalVertex vertex, MyBaseVertexCentricQuery baseQuery) {
        if (isPartitionedVertex(vertex)) {
    ...
    private Iterable<TitanVertex> executeIndividualVertices(InternalVertex vertex, MyBaseVertexCentricQuery baseQuery) {
        MyVertexCentricQuery query = constructQuery(vertex, baseQuery);
        if (useSimpleQueryProcessor(query, vertex)) return new MySimpleVertexQueryProcessor(query,tx).vertexIds();
        else return edges2Vertices((Iterable) executeIndividualRelations(vertex,baseQuery), query.getVertex());
    }

    public VertexList executeVertexIds(InternalVertex vertex, MyBaseVertexCentricQuery baseQuery) {
        if (isPartitionedVertex(vertex)) {
            //If there is a sort order, we need to first merge the relations (and sort) and then compute vertices
            if (!orders.isEmpty()) return edges2VertexIds((Iterable) executeRelations(vertex,baseQuery), vertex);
    ...
    private VertexList executeIndividualVertexIds(InternalVertex vertex, MyBaseVertexCentricQuery baseQuery) {
        MyVertexCentricQuery query = constructQuery(vertex, baseQuery);
        if (useSimpleQueryProcessor(query, vertex)) return new MySimpleVertexQueryProcessor(query,tx).vertexIds();
     ...
     * @return
     */
    protected MyVertexCentricQuery constructQuery(InternalVertex vertex, MyBaseVertexCentricQuery baseQuery) {
        ...
        MyVertexCentricQuery query = new MyVertexCentricQuery(vertex, condition, baseQuery.getDirection(), baseQuery.getQueries(),baseQuery.getOrders(), baseQuery.getLimit());
        Preconditions.checkArgument(!queryOnlyLoaded || query.isSimple(),"Query-only-loaded only works on simple queries");
        return query;
    }

    protected MyBaseVertexCentricQuery constructQuery(RelationCategory returnType) {
        QueryProfiler optProfiler = profiler.addNested(QueryProfiler.OPTIMIZATION);
        optProfiler.startTimer();
        MyBaseVertexCentricQuery query = constructQueryWithoutProfile(returnType);
        optProfiler.stopTimer();
        query.observeWith(profiler);
        return query;
    }

    protected MyBaseVertexCentricQuery constructQueryWithoutProfile(RelationCategory returnType) {
        ...
        if (limit <= 0)
            return MyBaseVertexCentricQuery.emptyQuery();
        ...
        List<BackendQueryHolder<EdgeSliceQuery>> queries;
        if (!hasTypes()) {
            BackendQueryHolder<EdgeSliceQuery> query = new BackendQueryHolder<EdgeSliceQuery>(serializer.getEdgeQuery(returnType, querySystem),
            ...
            queries = new ArrayList<BackendQueryHolder<EdgeSliceQuery>>(types.length + 2);
            ...
            for (Interval pint : intervalConstraints.values()) { //Check if one of the constraints leads to an empty result set
                if (pint.isEmpty()) return MyBaseVertexCentricQuery.emptyQuery();
            }
            ...
            for (String typeName : types) {
                ...
                if (type.isEdgeLabel() && typeDir==Direction.BOTH && intervalConstraints.isEmpty() && orders.isEmpty()) {
                    ...
                    EdgeSliceQuery q = serializer.getEdgeQuery(type, typeDir);
                    q.setLimit(sliceLimit);
                    queries.add(new BackendQueryHolder<EdgeSliceQuery>(q, isIntervalFittedConditions, true));
                } else {
        ...
        return new MyBaseVertexCentricQuery(QueryUtil.simplifyQNF(conditions), dir, queries, orders, limit);
    }
    ...
    private void constructSliceQueries(PropertyKey[] extendedSortKey, EdgeSerializer.TypedInterval[] sortKeyConstraints,
        ...
        EdgeSerializer serializer = tx.getEdgeSerializer();
        EdgeSliceQuery q = serializer.getEdgeQuery(bestCandidate, direction);
        q.setLimit(computeLimit(intervalConstraints.size()-position, sliceLimit));
        queries.add(new BackendQueryHolder<EdgeSliceQuery>(q, isFitted, bestCandidateSupportsOrder));
    }
