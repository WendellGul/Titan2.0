...
public class MetricInstrumentedStore implements KeyColumnValueStore {
    ...
    public MyEntryList getEdgeSlice(EdgeKeySliceQuery query, StoreTransaction txh) throws BackendException {
        return runWithMetrics(txh, metricsStoreName, M_GET_SLICE,
                new StorageCallable<MyEntryList>() {
                    public MyEntryList call() throws BackendException {
                        MyEntryList result = backend.getEdgeSlice(query, txh);
                        recordEdgeSliceMetrics(txh, result);
                        return result;
                    }
                }
        );
    ...
    public Map<Long, MyEntryList> getEdgeSlice(List<Long> keys, EdgeSliceQuery query, StoreTransaction txh) throws BackendException {
        return runWithMetrics(txh, metricsStoreName, M_GET_SLICE,
                new StorageCallable<Map<Long, MyEntryList>>() {
                    public Map<Long, MyEntryList> call() throws BackendException {
                        Map<Long, MyEntryList> results = backend.getEdgeSlice(keys, query, txh);

                        for (MyEntryList result : results.values()) {
                            recordEdgeSliceMetrics(txh, result);
                        }
                        return results;
                    }
    ...
    private void recordEdgeSliceMetrics(StoreTransaction txh, List<MyEntry> row) {
        if (!txh.getConfiguration().hasGroupName())
            return;
