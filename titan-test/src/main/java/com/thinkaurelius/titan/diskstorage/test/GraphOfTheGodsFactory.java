...
public class GraphOfTheGodsFactory {
    ...
    static void testEdges(TitanGraph g) {
        Iterable<TitanEdge> edges = g.query().edges();
        int i = 1;
        for(Edge e : edges) {
            String label = e.label();
            System.err.printf("%2d:%s-%s->%s", i++, e.outVertex().value("name"), label, e.inVertex().value("name"));
            for(String name : e.keys()) {
                Object s = e.value(name);
                System.err.printf("%7s:%7s\n", name, s);
            }
            System.err.println();
        }
    }

    static void testVertices(TitanGraph g) {
        Iterable<TitanVertex> vertices = g.query().vertices();
        int i = 1;
        for(Vertex v : vertices) {
            String label = v.label();
            System.err.printf("%2d(%s):%-8s", i++, v.id(), label);
            for(String name : v.keys()) {
                Object s = v.value(name);
                System.err.printf("%7s:%7s\n", name, s);
            }
            System.err.println();
        }
    ...
    private static void loadSample(TitanGraph graph) {
        //Create Schema
        TitanManagement mgmt = graph.openManagement();
        final PropertyKey name = mgmt.makePropertyKey("name").dataType(String.class).make();
        final PropertyKey age = mgmt.makePropertyKey("age").dataType(Integer.class).make();
