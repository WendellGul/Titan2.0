...
public class GraphOfTheGodsFactory {
    ...
    public static void main(String args[]) {
        ...
        load(g);
        Iterable<TitanVertex> vertices = g.query().has("name", "saturn").vertices();
        int i = 0;
        for(Vertex v : vertices) {
            System.err.println("Vertex ID: " + v.id());
            for(String key: v.keys()) {
                TitanVertexProperty property = (TitanVertexProperty) v.property(key);
                System.out.println("\t" + key + " :" + v.value(key) + " ID: " + property.id());
            }
        }
        g.close();
